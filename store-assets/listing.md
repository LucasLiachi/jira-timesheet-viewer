...
`store-assets/promo/small-tile-440x280.png` — generated by `scripts/make_small_tile.py`, same brand/icon but stacked (icon on top, name + short tagline below) since the small tile's canvas is too narrow for the large tile's side-by-side layout. Regenerate with:
